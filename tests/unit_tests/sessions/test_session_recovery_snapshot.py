...
def _build_service(
    ...
    *,
    ...
    return SessionService(
        ...
        background_terminal_repo=BackgroundTerminalRepository(db_path),
...
def test_get_recovery_snapshot_includes_background_terminals(tmp_path: Path) -> None:
    db_path = tmp_path / "recovery_background_terminals.db"
    service = _build_service(db_path)
    ...
    terminal_repo = BackgroundTerminalRepository(db_path)
    ...
    active_run = snapshot.get("active_run")
    assert isinstance(active_run, dict)
    assert active_run.get("background_terminal_count") == 1
    background_terminals = snapshot.get("background_terminals")
    assert isinstance(background_terminals, list)
    assert len(background_terminals) == 1
    assert background_terminals[0]["terminal_id"] == "term-1"
    round_snapshot = snapshot.get("round_snapshot")
    assert isinstance(round_snapshot, dict)
    assert round_snapshot.get("background_terminal_count") == 1
