...
class TestIsBinaryFile:
    ...
    def test_binary_extension_zip(self, tmp_path):
        """测试 zip 扩展名"""
        from agent_teams.tools.workspace.read import is_binary_file
...
class TestReadFileContent:
    ...
    async def test_read_file_all_lines(self, tmp_path):
        """测试读取所有行"""
        from agent_teams.tools.workspace.read import read_file_content
...
class TestReadDirectory:
    ...
    def test_read_directory(self, tmp_path):
        """测试目录读取"""
        from agent_teams.tools.workspace.read import read_directory
