...
class TestArtifactRepoEdgeCases:
    """Cover JSON decode error paths and type coercion in _row_to_entry."""

    def test_row_to_entry_bad_json_in_payload(self) -> None:
        import tempfile
        from pathlib import Path
        ...
        with tempfile.TemporaryDirectory() as td:
            repo = TaskArtifactRepository(Path(td) / "test.db")
            # Insert a row with invalid JSON payload_json
            import sqlite3

            conn = sqlite3.connect(str(Path(td) / "test.db"))
            conn.execute(
                "INSERT INTO task_artifact_entries "
                "(entry_id, task_id, phase, timestamp, role_id, instance_id, "
                "event_type, description, payload_json, linked_evidence_ids) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    "e-bad",
                    "t-1",
            ...
            conn.commit()
            conn.close()
            entries, total = repo.query_entries(task_id="t-1")
            assert total == 1
            assert entries[0].payload_json == "not-valid-json{"

    def test_row_to_entry_linked_evidence_ids_list(self) -> None:
        import tempfile
        import sqlite3
        from pathlib import Path
        from relay_teams.agents.tasks.artifact_repository import TaskArtifactRepository
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            conn.commit()
            conn.close()
            entries, _ = repo.query_entries(task_id="t-2")
            assert entries[0].linked_evidence_ids == ("art-1", "art-2")

    def test_row_to_entry_linked_evidence_ids_bad_json(self) -> None:
        import tempfile
        import sqlite3
        from pathlib import Path
        from relay_teams.agents.tasks.artifact_repository import TaskArtifactRepository
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            conn.commit()
            conn.close()
            entries, _ = repo.query_entries(task_id="t-3")
            assert entries[0].linked_evidence_ids == ()

    def test_row_to_entry_payload_dict(self) -> None:
        import tempfile
        import sqlite3
        from pathlib import Path
        from relay_teams.agents.tasks.artifact_repository import TaskArtifactRepository
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            conn.commit()
            conn.close()
            entries, _ = repo.query_entries(task_id="t-4")
            import json

            assert json.loads(entries[0].payload_json)["key"] == "val"
