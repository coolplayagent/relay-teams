...
from agent_teams_evals.workspace.base import WorkspaceSetupError
from agent_teams_evals.workspace.docker_setup import (
    DockerConfig,
...
def test_ensure_instance_image_passes_through_original_setup_repo_script(
    ...
    monkeypatch.setattr(
        "agent_teams_evals.workspace.docker_setup._install_resource_stub",
        lambda: None,
    )
    monkeypatch.setattr(
        "agent_teams_evals.workspace.docker_setup._load_docker_module",
        lambda: _FakeDockerModule(),
    )
    monkeypatch.setattr(
        "agent_teams_evals.workspace.docker_setup._configure_swebench_log_dirs",
        lambda: constants,
    )
    monkeypatch.setattr(
        "agent_teams_evals.workspace.docker_setup._load_swebench_docker_build_module",
        lambda: build_module,
    )
    monkeypatch.setattr(
        "agent_teams_evals.workspace.docker_setup._load_swebench_test_spec_module",
        lambda: _FakeTestSpecModule(spec),
    )
    monkeypatch.setattr(
        "agent_teams_evals.workspace.docker_setup._load_dataset",
        lambda path, *, split, streaming=False: [{"instance_id": "demo"}],
    )
    monkeypatch.setattr(
        "agent_teams_evals.workspace.docker_setup._image_exists",
        _image_exists,
    )
