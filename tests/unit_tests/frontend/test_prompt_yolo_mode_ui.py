...
def test_chat_input_renders_yolo_toggle() -> None:
    html = Path("frontend/dist/index.html").read_text(encoding="utf-8")

    assert 'id="yolo-mode-toggle"' in html
    assert "YOLO 模式" in html


def test_send_user_prompt_includes_approval_mode(tmp_path: Path) -> None:
...
await sendUserPrompt("session-1", "ship it", "yolo");
