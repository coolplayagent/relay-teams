...
def evaluate_feedback_loop(
    ...
    converged = len(unmet_acceptance_criteria) == 0 and len(unmet_verification_points) == 0
...
def evaluate_feedback_loop_recursively(
    ...
    for index, signals in enumerate(bounded_history, start=1):
        evaluation = evaluate_feedback_loop(spec, observed_signals=signals, iteration=index)
