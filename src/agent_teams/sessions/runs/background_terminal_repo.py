# -*- coding: utf-8 -*-
from __future__ import annotations

import json
import sqlite3
from datetime import datetime, timezone
from pathlib import Path
from threading import RLock

from agent_teams.persistence.db import open_sqlite, run_sqlite_write_with_retry
from agent_teams.sessions.runs.background_terminal_models import (
    BackgroundTerminalRecord,
    BackgroundTerminalStatus,
)
from agent_teams.validation import (
    normalize_persisted_text,
    parse_persisted_datetime_or_none,
    require_persisted_identifier,
)


class BackgroundTerminalRepository:
    def __init__(self, db_path: Path) -> None:
        self._db_path = Path(db_path)
        self._conn = open_sqlite(db_path)
        self._conn.row_factory = sqlite3.Row
        self._lock = RLock()
        self._init_tables()

    def _init_tables(self) -> None:
        def operation() -> None:
            self._conn.execute(
                """
                CREATE TABLE IF NOT EXISTS background_terminals (
                    terminal_id      TEXT PRIMARY KEY,
                    run_id           TEXT NOT NULL,
                    session_id       TEXT NOT NULL,
                    instance_id      TEXT,
                    role_id          TEXT,
                    tool_call_id     TEXT,
                    command          TEXT NOT NULL,
                    cwd              TEXT NOT NULL,
                    execution_mode   TEXT NOT NULL,
                    status           TEXT NOT NULL,
                    tty              INTEGER NOT NULL,
                    timeout_ms       INTEGER,
                    exit_code        INTEGER,
                    recent_output_json TEXT NOT NULL,
                    stdout_tail_json TEXT NOT NULL,
                    stderr_tail_json TEXT NOT NULL,
                    log_path         TEXT NOT NULL,
                    created_at       TEXT NOT NULL,
                    updated_at       TEXT NOT NULL,
                    completed_at     TEXT
                )
                """
            )
            self._conn.execute(
                """
                CREATE INDEX IF NOT EXISTS idx_background_terminals_run
                ON background_terminals(run_id, updated_at DESC)
                """
            )
            self._conn.execute(
                """
                CREATE INDEX IF NOT EXISTS idx_background_terminals_status
                ON background_terminals(status, updated_at DESC)
                """
            )

        run_sqlite_write_with_retry(
            conn=self._conn,
            db_path=self._db_path,
            operation=operation,
            lock=self._lock,
            repository_name="BackgroundTerminalRepository",
            operation_name="init_tables",
        )

    def upsert(self, record: BackgroundTerminalRecord) -> BackgroundTerminalRecord:
        def operation() -> None:
            self._conn.execute(
                """
                INSERT INTO background_terminals(
                    terminal_id,
                    run_id,
                    session_id,
                    instance_id,
                    role_id,
                    tool_call_id,
                    command,
                    cwd,
                    execution_mode,
                    status,
                    tty,
                    timeout_ms,
                    exit_code,
                    recent_output_json,
                    stdout_tail_json,
                    stderr_tail_json,
                    log_path,
                    created_at,
                    updated_at,
                    completed_at
                )
                VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(terminal_id)
                DO UPDATE SET
                    run_id=excluded.run_id,
                    session_id=excluded.session_id,
                    instance_id=excluded.instance_id,
                    role_id=excluded.role_id,
                    tool_call_id=excluded.tool_call_id,
                    command=excluded.command,
                    cwd=excluded.cwd,
                    execution_mode=excluded.execution_mode,
                    status=excluded.status,
                    tty=excluded.tty,
                    timeout_ms=excluded.timeout_ms,
                    exit_code=excluded.exit_code,
                    recent_output_json=excluded.recent_output_json,
                    stdout_tail_json=excluded.stdout_tail_json,
                    stderr_tail_json=excluded.stderr_tail_json,
                    log_path=excluded.log_path,
                    created_at=excluded.created_at,
                    updated_at=excluded.updated_at,
                    completed_at=excluded.completed_at
                """,
                _record_params(record),
            )

        run_sqlite_write_with_retry(
            conn=self._conn,
            db_path=self._db_path,
            operation=operation,
            lock=self._lock,
            repository_name="BackgroundTerminalRepository",
            operation_name="upsert",
        )
        persisted = self.get(record.terminal_id)
        if persisted is None:
            raise RuntimeError(
                f"Failed to persist background terminal {record.terminal_id}"
            )
        return persisted

    def get(self, terminal_id: str) -> BackgroundTerminalRecord | None:
        with self._lock:
            row = self._conn.execute(
                """
                SELECT *
                FROM background_terminals
                WHERE terminal_id=?
                """,
                (terminal_id,),
            ).fetchone()
        if row is None:
            return None
        return _row_to_record(row)

    def list_by_run(self, run_id: str) -> tuple[BackgroundTerminalRecord, ...]:
        with self._lock:
            rows = self._conn.execute(
                """
                SELECT *
                FROM background_terminals
                WHERE run_id=?
                ORDER BY updated_at DESC, created_at DESC
                """,
                (run_id,),
            ).fetchall()
        return tuple(_row_to_record(row) for row in rows)

    def mark_transient_terminals_interrupted(self) -> int:
        affected = 0

        def operation() -> None:
            nonlocal affected
            now = datetime.now(tz=timezone.utc).isoformat()
            cursor = self._conn.execute(
                """
                UPDATE background_terminals
                SET status=?, updated_at=?, completed_at=COALESCE(completed_at, ?)
                WHERE status IN (?, ?)
                """,
                (
                    BackgroundTerminalStatus.STOPPED.value,
                    now,
                    now,
                    BackgroundTerminalStatus.RUNNING.value,
                    BackgroundTerminalStatus.BLOCKED.value,
                ),
            )
            affected = int(cursor.rowcount or 0)

        run_sqlite_write_with_retry(
            conn=self._conn,
            db_path=self._db_path,
            operation=operation,
            lock=self._lock,
            repository_name="BackgroundTerminalRepository",
            operation_name="mark_transient_terminals_interrupted",
        )
        return affected


def _record_params(record: BackgroundTerminalRecord) -> tuple[object, ...]:
    return (
        record.terminal_id,
        record.run_id,
        record.session_id,
        record.instance_id,
        record.role_id,
        record.tool_call_id,
        record.command,
        record.cwd,
        record.execution_mode,
        record.status.value,
        1 if record.tty else 0,
        record.timeout_ms,
        record.exit_code,
        json.dumps(record.recent_output, ensure_ascii=False),
        json.dumps(record.stdout_tail, ensure_ascii=False),
        json.dumps(record.stderr_tail, ensure_ascii=False),
        record.log_path,
        record.created_at.isoformat(),
        record.updated_at.isoformat(),
        record.completed_at.isoformat() if record.completed_at is not None else None,
    )


def _row_to_record(row: sqlite3.Row) -> BackgroundTerminalRecord:
    created_at = parse_persisted_datetime_or_none(row["created_at"])
    updated_at = parse_persisted_datetime_or_none(row["updated_at"])
    if created_at is None or updated_at is None:
        raise ValueError("Invalid persisted terminal timestamps")
    return BackgroundTerminalRecord(
        terminal_id=require_persisted_identifier(
            row["terminal_id"], field_name="terminal_id"
        ),
        run_id=require_persisted_identifier(row["run_id"], field_name="run_id"),
        session_id=require_persisted_identifier(
            row["session_id"], field_name="session_id"
        ),
        instance_id=normalize_persisted_text(row["instance_id"]),
        role_id=normalize_persisted_text(row["role_id"]),
        tool_call_id=normalize_persisted_text(row["tool_call_id"]),
        command=str(row["command"]),
        cwd=str(row["cwd"]),
        execution_mode="background",
        status=BackgroundTerminalStatus(str(row["status"])),
        tty=bool(int(row["tty"])),
        timeout_ms=int(row["timeout_ms"]) if row["timeout_ms"] is not None else None,
        exit_code=int(row["exit_code"]) if row["exit_code"] is not None else None,
        recent_output=_decode_lines(row["recent_output_json"]),
        stdout_tail=_decode_lines(row["stdout_tail_json"]),
        stderr_tail=_decode_lines(row["stderr_tail_json"]),
        log_path=str(row["log_path"]),
        created_at=created_at,
        updated_at=updated_at,
        completed_at=parse_persisted_datetime_or_none(row["completed_at"]),
    )


def _decode_lines(value: object) -> tuple[str, ...]:
    normalized = normalize_persisted_text(value)
    if normalized is None:
        return ()
    try:
        decoded = json.loads(normalized)
    except json.JSONDecodeError:
        return ()
    if not isinstance(decoded, list):
        return ()
    result: list[str] = []
    for item in decoded:
        if isinstance(item, str) and item.strip():
            result.append(item)
    return tuple(result)
