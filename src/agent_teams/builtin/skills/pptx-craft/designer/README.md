...
### 阶段 4.3：布局 QA（必须在转 PPTX 前执行）

在生成所有成品 HTML 页面后，必须先执行基于 JS 的布局 QA，再进行 PPTX 转换。
...
4. **强制检查清单**：
   ...
   - 对于带边框卡片/面板，必须重点关注文字是否贴边、压线、被裁切；必要时重新排版后再次运行 JS QA

5. **执行要求**：
   - 这是基于 JS 的强制前置检查，不得口头自检后跳过
   - 只要 QA 结果存在 `FAIL`、退出码非 0、或 JSON 报告显示未通过，就不得进入 PPTX 转换
   - 修复后必须重新执行 `node designer/slide_layout_qa.js {output_dir}`，直到通过为止
...
**5.1 文本装盒与边框安全区（新增强制规则）**

- 任何带边框的卡片、面板、表格单元、摘要框，正文必须预留内边距，不得让文字贴边
- 卡片类模块默认内边距建议：左右 ≥ 16px、上下 ≥ 14px；高密度页面可缩小，但不得低于左右 12px、上下 10px
- 带序号、大号数字、标题和列表共存时，必须优先通过“增加换行、缩短单行长度、拆分段落、调整列宽/卡片高度”来消化内容，字号与行高缩小只能作为后备手段，禁止一味靠缩字解决
- 当页面为三列或更多列布局时，正文建议优先使用 14-16px，行高 1.45-1.6；若单卡片文字超过 3 条，应优先改写为两行标题、短句 bullet、独立摘要框或减少同屏卡片数量
- 如文本信息过密，优先考虑把纯文字列表转换为图表、趋势图、对比条、流程图、矩阵卡或示意图，用可视化分担文字压力
- 生成后必须自检：最外层边框与任一文本的最小可视距离应明显大于 0，禁止出现“文字压线、出框、半截被裁”现象
- 如内容较满，优先新增独立摘要区或降低装饰占比，不得牺牲正文可见性
...
- 当页面底部存在 summary / bottom band / footer note 时，必须预留独立安全区；主内容区不得与底部总结条发生垂直重叠
- 若页面采用“主体内容 + 底部总结条”结构，应优先压缩总结条高度或上移主体边界，不得让正文最后一行与总结条投影区域相交
- 布局 QA 必须检查 summary band collision：只要主内容文字与 summary / bottom band / tail 区域发生投影相交，就应判定为错误，而不能通过白名单直接跳过
- text_safe_padding 与底部 band / 第三卡片区域的 recursive_blank 也属于强约束项：若关键信息模块因布局导致可用内容区不足、出现高比例局部空白或文字贴边，应视为不通过，而不是可接受 warning
- 使用半透明遮罩时确保不影响文字可读性
...
4. **运行布局 QA**：
   ...
   - 这是基于 JS 的强制前置检查，不得跳过
   - 检查终端输出是否存在 `FAIL`
   - 检查 `path.dirname({output_dir})/layout_qa_result.json` 是否生成
   - 若存在错误项，先修复页面，再重新运行 JS QA，确认通过后才能进入 PPTX 转换
