...
ROLE_USAGE_PROMPT = (
    "## Role Usage\n"
    "You have three orchestration tools. Each role entry below is a dispatch target, "
    "not one of Coordinator's own capabilities.\n\n"
    "### create_tasks\n"
    "Create one or more tasks and assign each to a role from the Available Roles list.\n"
    "- `tasks` (required): list of `{role_id, objective, title?}`. "
    "Put the selected role id into `role_id` and write a concrete objective for that role.\n"
    "- `auto_dispatch` (optional, default false): when true, immediately dispatch a single task "
    "after creation. Only allowed when `tasks` contains exactly one entry.\n"
    "- Returns created task ids and statuses; use the returned `task_id` for subsequent calls.\n\n"
    "### dispatch_task\n"
    "Start execution of a created task, or re-dispatch a completed task with feedback.\n"
    "- `task_id` (required): the id returned by `create_tasks`.\n"
    "- `feedback` (optional): additional instructions appended to the task conversation. "
    "Required when re-dispatching a completed task for follow-up work.\n"
    "- If the task has failed or timed out, the error message explains what went wrong; "
    "use `create_tasks` to create a replacement task instead of re-dispatching.\n"
    "- If the target role is busy with another task, "
    "the error message identifies the blocking task; wait for it to complete.\n\n"
    "### update_task\n"
    "Modify a task that is still in `created` status (not yet dispatched).\n"
    "- `task_id` (required): the task to update.\n"
    "- `role_id`, `objective`, `title` (all optional): fields to change. "
