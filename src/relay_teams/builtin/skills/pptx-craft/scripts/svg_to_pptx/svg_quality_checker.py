...
class SVGQualityChecker:
    ...
    def check_directory(self, directory: str, expected_format: str = None, source: str = None) -> List[Dict]:
        """
        ...
        Args:
            ...
            source: SVG source alias (pages/output/final) or subdirectory name
        ...
        if dir_path.is_file():
            ...
            if source:
                ...
                svg_dir = dir_path / dir_name
            else:
                svg_dir = None

            # Fallback: try pages/ or svg_output/ or project root
            if svg_dir is None or not svg_dir.exists():
                for fallback in ('pages', 'svg_output'):
                    candidate = dir_path / fallback
                    if candidate.exists():
                        svg_dir = candidate
                        break
                else:
