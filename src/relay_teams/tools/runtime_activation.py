...
def merge_active_tools(
    *,
    ...
    initial_active = build_initial_active_tools(
        authorized,
        always_active_tools=always_active_tools,
    )
    normalized_existing_active = validate_activation_request(
        authorized_tools=authorized,
        active_tools=active_tools,
        requested_tool_names=(),
    ).active
    return validate_activation_request(
        authorized_tools=authorized,
        active_tools=initial_active + normalized_existing_active,
        requested_tool_names=(),
    ).active
...
def build_initial_active_tools(
    ...
    *,
    ...
    required_always_active_tools = _dedupe_names(always_active_tools)
    initial_active_tools = tuple(
        name for name in required_always_active_tools if name in authorized_set
    )
    if required_always_active_tools and len(initial_active_tools) == len(
        required_always_active_tools
    ):
