...
- **32 文件变更**，分三类：
  - **Service 层（11 文件）**：为 10 个 Service 类补全 `*_async` 方法（使用 `asyncio.to_thread` 包装同步方法），包括 SessionService（27）、TriggerService（21）、RoleSettingsService（7）、WorkspaceService（14+）、FeishuGatewayService（7）、WeChatGatewayService（7）、XiaolubanGatewayService（7）、SpeechConfigService（2）、AssetService（5）。FeishuSubscriptionService 已从 `asyncio.to_thread` 包装模式升级为原生 async（`start`/`stop`/`reload` 全部 `async def`，`reload_async()` 已删除，`_FeishuWsHub` 从专用 daemon 线程改为纯 async 运行在主事件循环上）。
