...
| **所属维度** | Spec-Driven 流程 |
| **优先级** | **低**（核心机制已在 SP-1 中落地，仅剩增强项） |

#### 当前状态

**已部分落地（2026-05-02）**。SP-3 提出的两条核心策略——"规格认知刷新"与"规格优先压缩"——均已有对应实现：
...
| 能力 | 落地实现 | 关键代码位置 |
|------|----------|-------------|
| **Spec Checkpoint Policy** | `SpecCheckpointPolicy` 配置 `max_tool_calls` / `max_messages` / `max_history_tokens` 三阈值，控制自动刷新触发条件 | `agents/tasks/models.py` L210 |
...
仍存在的增强方向（非原始缺口，属于后续优化）：

- `render_spec_checkpoint()`（`spec_checkpoint.py` L126）目前仅刷新 baseline 字段，不包含 REASONS Canvas 字段和 formal verification 详情——可扩展为覆盖完整结构化 spec（与 SP-1 后续增强方向一致）。
...
SP-3 的核心机制已在 SP-1 中落地。建议后续增强方向：

1. 扩展 `render_spec_checkpoint()` 覆盖 REASONS Canvas 字段（entities / approach / structure / operations / norms / safeguards）和 `FormalVerificationPlan` 详情，使长周期任务的 spec refresh 刷新完整结构化 spec。
2. `SpecCheckpointPolicy` 可考虑增加基于 spec 变更号的增量刷新机制——当 `TaskSpecArtifact.version` 递增时触发立即刷新，而非仅依赖固定阈值。
...
**完成情况**：SP-1 已于 2026-05-02 完成形式化规格嵌入闭环（六大缺口全部闭合，含 spec artifact 一等工件、spec lineage tracking、normalized Evidence Bundle、REASONS Canvas、prompt/code 同步、形式化验证证据绑定）；FE-5 已于 2026-05-02 完成八层验证管线闭环。后续增强方向：扩展 spec checkpoint 渲染、建立 spec artifact 版本链/diff API、前端 lineage 可视化、接入 LLM evaluator。
