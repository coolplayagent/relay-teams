...
已确立"异步优先"架构原则，并完成编排核心路径第一阶段迁移、automation 管理面第二阶段迁移、automation delivery/queue 第三阶段迁移，以及剩余业务/运行时仓储收尾迁移。后续应保持 `tests/unit_tests/test_async_wrapper_coverage.py` 的仓库级断言，禁止 `src/relay_teams` 业务模块重新调用 `_call_sync_async`；同步入口仅应保留在 CLI 等必要边界，通过 `asyncio.run()` 调用 async API。
