...
**已完成闭环（2026-05-01）**。经过四阶段渐进迁移，async 路径已全面统一为原生 aiosqlite 实现。

第一阶段（2026-04-27）：编排热路径已改为 async 优先：
...
- 收尾阶段已落地（2026-05-01）。剩余业务/运行时仓储 async 方法已统一为原生 async SQLite 路径，覆盖 message history、task/run 批量查询、external sessions、gateway account/queue/pool、workspace、media assets、triggers、monitors、retrieval FTS、role memory、token usage、approval/question/background runtime 等持久化面。`_call_sync_async` 现在只保留在 `SharedSqliteRepository` 作为兼容桥实现，`src/relay_teams` 其他模块没有运行时调用；新增仓库级静态测试会扫描源码并阻止同步桥调用回流。

仓库中仍有面向 CLI、脚本或传统同步调用方的同步 API，但 runtime/server/仓储 async 路径不再通过 `_call_sync_async` 包装同步 SQLite 方法。后续清理重点应从 AO-4 的同步桥迁移转向逐步收敛冗余同步 API、管理面 router 的 `call_maybe_async` 兼容路径，以及只在 CLI/进程入口保留必要的 `asyncio.run()` 边界。

**同步 API 收敛路线**：(a) CLI 入口的 `asyncio.run()` 边界定义标准——每个 CLI command 只在顶层调用一次 `asyncio.run()`，内部全部走 async 路径；(b) `call_maybe_async` 在 router 层的退出计划——逐步将 `interfaces/server/routers/` 中残留的 `call_maybe_async` 调用替换为直接 `await`，目标是 router 层全部 async 原生；(c) SDK 同步 API 保留评估——`interfaces/sdk/` 的同步 API 是否保留取决于外部调用方需求，可考虑提供 `AsyncSDK` 和 `SyncSDK` 两个入口。
...
已确立"异步优先"架构原则，并完成编排核心路径第一阶段迁移、automation 管理面第二阶段迁移、automation delivery/queue 第三阶段迁移，以及剩余业务/运行时仓储收尾迁移。后续应保持 `tests/unit_tests/test_async_wrapper_coverage.py` 与 `tests/unit_tests/automation/test_automation_repository.py` 的仓库级断言，禁止 `src/relay_teams` 业务模块重新调用 `_call_sync_async`；同步入口仅应保留在 CLI 等必要边界，通过 `asyncio.run()` 调用 async API。
...
存在 `conversation_compaction.py`（经验证达 1146 行，压缩机制较完善），但**没有 Prompt Caching（缓存）、Context Editing（编辑）策略**（`tools/runtime/` 目录下无缓存或编辑模块）。每次 LLM 调用都重新构建完整 system_prompt，即使角色定义和技能描述等静态内容在多次调用间不变。上下文管理缺少战略层级的设计。
...
FE-5 的验证引擎已升级为完整的八层管线（`verification.py` 中 `_run_strictness_checks()` 和各层专用检查方法），由 `TaskSpecStrictness` 驱动检查深度：
...
> 以下基于 2026-05-03 进度重新评估。AO-1、AO-2、SP-1、SG-2 均已闭环，推荐重心已转移。

### 1. EP-1：全面 Context Engineering 策略（成本低、收益高）

**完成情况**：compaction 已完善（1146 行），Provider-native prompt caching 尚未接入。当前主流 Provider（Anthropic / OpenAI）已在 API 层原生支持 prompt caching，接入成本极低。

**后续收益**：标记 system_prompt 前缀和工具 schema 为可缓存段，预计直接减少 20-40% 重复 Token 处理开销。这是 Phase 2 中仅剩的两个主要未完成项之一。

### 2. OP-9：Harness 控制面与 Sandbox 计算面分离

**为什么紧迫**：AO-1 的 Harness 解构和 SG-1 的运行时护栏均已落地，但控制面（编排/审批/追踪/恢复/run state）与计算面（sandbox/worktree/remote workspace）的边界仍不够显式。`task_execution_service.py` 仍有 1833 行，大部分是控制面逻辑。

**预期收益**：明确分离后，安全策略不会泄漏到可被 Agent 修改的执行环境，为 Phase 3 的外部任务集成（OP-11 Task Board、OP-1 Wake Queue）扫清障碍。

### 3. FE-1：跨 Run 的 Memory Bank（核心差异化）

**为什么紧迫**：这是 Phase 3 中可独立启动的高价值特性，学术基础已充分（六种记忆操作 + 三层分类 + Mem0 工程数据）。relay-teams 已有 BM25 记忆（`memory_bm25.py`）和角色记忆服务作为基础。
...
1. **架构层面**：TaskExecutionService 的 Harness 解构（AO-1）和 DAG 编排（AO-2）均已完成，架构基础已扫清主要障碍；同步/异步统一（AO-4）已全面迁移为 async aiosqlite 原生路径
2. **流程层面**：Spec-Driven Development 的形式化嵌入（SP-1）六大缺口已全部闭合；八层验证管线（FE-5）已闭环；RoleContract（SP-2）已结构化角色行为合约；SP-3 增强项 E1-E4 全部完成
3. **安全层面**：角色行为边界（SG-2）已落地强制执行；运行时护栏（SG-1）基础框架已实现三层/六规则/三动作模型
4. **工程层面**：全面的 Context Engineering（EP-1）仍是 Phase 2 仅剩的主要缺口，Provider-native prompt caching 提供低成本快速落地路径
5. **功能层面**：跨 Run 的 Memory Bank（FE-1）有坚实学术基础支持（六种记忆操作 + 三层分类 + Mem0 数据），是下一阶段核心差异化特性
6. **运营层面**：事件驱动基础设施（Triggers + Monitors + Automation）已完整实现，为 OP-11 Task Board 和 OP-1 Wake Queue 提供了基础设施前身
