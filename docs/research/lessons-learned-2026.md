...
仓库中仍存在非编排热路径的 sync/async 双路径，主要分布在 session/admin/gateway 以及 automation delivery/queue 等外部配置和管理面。后续阶段应继续将这些边界迁移为 async 服务方法，并把同步入口限制在 CLI/进程入口。
...
已确立"异步优先"架构原则，并完成编排核心路径第一阶段迁移与 automation 管理面第二阶段迁移。后续应按仓储归属继续清理剩余 `_call_sync_async` 包装器和 server router 的 `call_maybe_async` 管理面桥接；同步入口仅应保留在 CLI 等必要边界，通过 `asyncio.run()` 调用 async API。
