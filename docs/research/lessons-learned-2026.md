...
**已完成闭环（2026-04-29，Phase 2 收敛 2026-05-05）**。`TaskExecutionService` 已按 Harness 模式拆分，主执行路径已从服务私有兼容方法迁移到 Harness 公共方法；**Phase 1**（2026-04-29）完成了主执行路径切换和 AutoHarness 闭环；**Phase 2**（2026-05-05）完成了控制面逻辑全量迁移，`task_execution_service.py` 从 1420 行缩减至 **832 行**（-588，-41.4%），`control_harness.py` 从 275 行扩展至 **960 行**（+685，3.49x）。原先混杂在单文件中的职责已迁移到 `src/relay_teams/agents/orchestration/harnesses/`，并补齐 AutoHarness 生成工具闭环：
...
**Phase 2 收敛已完成（2026-05-05）**。`TaskControlHarness` 已承载全量控制面能力，`task_execution_service.py` 缩减为 832 行的纯编排协调者。具体变更：

- 8 个控制面方法从 `TaskExecutionService` 迁移到 `TaskControlHarness`：`transition_task_to_running`、`initialize_task_artifact`、`publish_guardrail_report`、`complete_task_timeout`、`complete_timeout_after_worker_cancel`、`persist_cancelled_execution`、`wait_for_worker_with_progress_timeout`、扩展版 `start_heartbeat`
- `TaskControlHarness` 构造参数扩展：新增 `message_repo`、`shared_store`、`run_event_hub`、`run_control_manager`
- `coordinator.py` 消除 `isinstance(TaskExecutionService)` 绕过检查，改用 `getattr` 安全访问
- `_timeout_handoff` 双份实现合并为 ControlHarness 中的单一份（保留 `task.handoff.model_copy()` 完整逻辑）
- `TaskExecutionServiceLike.execute()` Protocol 签名完全不变，8 个消费者零影响
- 新增 22 个 Phase 2 单元测试用例覆盖所有迁移方法
- 验证：basedpyright 零错误，pytest 849 passed
...
**Phase 6（2026-05-05）**：三项收敛方向全部落地。(a) CLI 入口已确认为 `asyncio.run()` 顶层边界，`_async_sync_lifecycle_calls()` 扫描范围从 `interfaces/server` 扩展至 `interfaces/cli`，确保 async 方法不调用同步生命周期方法。(b) `SyncAgentTeamsClient` 已作为 `AsyncAgentTeamsClient` 的同步包装器落地，所有方法通过 `asyncio.run` 委托 async API，`stream_run_events` 适配为返回 `list`。包导出 `from relay_teams import SyncAgentTeamsClient`。(c) `call_maybe_async` 已标记为 deprecated，调用时触发 `DeprecationWarning`，指引调用方使用显式 async service 方法或专用 route-work 变体。`api-design.md` 已补全 Client SDK 文档。
