...
| 治理与 AI-native 组织 | [`hello/The_Playbook_For_Building_An_AI_Native_Company.md`](lessons-learned/hello/The_Playbook_For_Building_An_AI_Native_Company.md), [`hello/AGENTS.md`](lessons-learned/hello/AGENTS.md), [`hello/CLAUDE.md`](lessons-learned/hello/CLAUDE.md), [`hello/GEMINI.md`](lessons-learned/hello/GEMINI.md) | 闭环组织、可查询工件、跨 Provider 治理包、证据纪律 | 校准产品层治理和组织级知识工程 |
...
| **OP-8** | 跨 Provider 治理包与 A2A 五元组 handoff | 中 | [`hello/AGENTS.md`](lessons-learned/hello/AGENTS.md)、[`hello/CLAUDE.md`](lessons-learned/hello/CLAUDE.md)、[`hello/GEMINI.md`](lessons-learned/hello/GEMINI.md) | relay-teams 能读取这些文件，但没有统一生成和版本化治理包 | 将硬约束、协作标准、质量纪律、知识工程拆成版本化 governance pack；A2A/role handoff 统一 What/Why/Tradeoff/Open Questions/Next Action |
...
仍未闭合的部分是：Designer 产出的规格还不是独立可引用的一等持久工件；任务之间没有记录"该 spec 来源于哪个上游 artifact"；Gater 虽能看到 verification/evidence 字段，但还没有强制读取 normalized Evidence Bundle，也没有语义级 spec compliance 判定。

#### 对比价值

这是解决"规格存在但证据链不够硬"的关键改进。relay-teams 已经有结构化 `TaskSpec`，下一步应把规格从 prompt 附件提升为可追溯的任务工件，并让执行、验证、handoff 都围绕同一份规格引用运行。
...
在现有 `TaskSpec` 基础上增加 `spec_artifact_id` 或 `spec_source_task_id`，建立"规格→执行→验证→证据包"的闭环：(1) Designer 阶段产出的规格自动转为持久 artifact 并绑定到后续 TaskEnvelope；(2) Crafter prompt 继续注入规格全文，但输出必须引用 acceptance criteria 和 evidence expectations；(3) Gater 读取 normalized Evidence Bundle，而不是只读上游自然语言结果；(4) strictness=high 时要求 verification commands 或结构化断言。参照 Piskala 的三级严格度：默认中等严格度，简单任务低严格度，安全关键任务高严格度。

#### 可行性评估

源码验证 `TaskEnvelope.spec`、`TaskSpec`、`VerificationPlan.acceptance_criteria`、`TaskPromptHarness.task_contract_prompt()` 已存在。剩余工作主要是 artifact 追踪和证据包串联，**可行性：高**。Pydantic v2 模型扩展+流程串联，2-3 周可交付。风险点：旧 TaskRecord 需 Optional 兼容，且前端/API 需要展示 spec 来源和验证证据。
...
本研究通过对 2026 年 AI Agent 领域 38 篇前沿研究的系统分析，识别出 25 个与 relay-teams 产品高度相关的改进借鉴点。这些借鉴点分布在六大维度，覆盖了从底层架构到顶层功能的完整技术栈。2026-04-28/29 增补又对 hello 项目中 1226 个关键词命中的 AI Markdown 文件和 OpenAI Symphony 状态机研究做复盘，并把本文件引用的 hello 来源材料归档到 [`lessons-learned/hello/`](lessons-learned/hello/)，补充了 11 个更偏运营化、产品化和跨 Provider 协作的 OP 借鉴点。
...
*本文件整合自 cross-reference-analysis.md（25 借鉴点）、markdown-research-points.md（35 研究点 + 38 源文件）、validation-report.md（验收修正）三份原始报告，并在 2026-04-28/29 增补 hello AI Markdown 与 OpenAI Symphony 状态机复盘；本文件直接引用的 hello 来源材料已复制到 `lessons-learned/hello/`。所有原 25 个借鉴点已逐一提取、交叉引用来源、标注验收修正；新增 OP 借鉴点以当前源码状态为准。*
