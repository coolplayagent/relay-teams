...
2026-04-29 同步复盘 hello 项目的 [`docs/formal-verification-research/`](lessons-learned/hello/docs/formal-verification-research/) 后，补充一个面向高严格度任务的结论：relay-teams 的验证层不应只依赖命令输出和自然语言 Gater 判断，而应为关键状态机、协议、调度、预算和安全策略提供可选的轻量形式化验证通道。TLA+/Alloy 适合先验证状态转移和不变式，Lean/Coq/Isabelle 适合承载数学、算法或高价值规约证明；这些机器可检查证据可以成为 `VerificationPlan` 与 Evidence Bundle 的高可信输入。
...
5. **增补复盘**：2026-04-28 对 hello 项目的 AI Markdown 语料做二次分组分析，并把本文件引用的来源副本放入 [`lessons-learned/hello/`](lessons-learned/hello/)；分析优先采用 `docs/coding-agent-collaboration-*.md`、`docs/reports/2026/{harness,sdd,mas,google}/` 等汇总材料，再回查 relay-teams 当前源码确认已落地与缺口
...
7. **SPDD 复盘**：2026-04-29 追加复盘 hello `docs/spdd-report/`，将 Thoughtworks SPDD、OpenSPDD CLI、SDD 生态对照映射到 relay-teams 的 TaskSpec、prompt 持久化、证据包和审查闭环缺口
8. **形式化验证复盘**：2026-04-29 追加复盘 hello `docs/formal-verification-research/`，将 TLA+、Alloy、Lean、Coq、Isabelle、seL4、CompCert 等形式化方法经验映射到 relay-teams 的高严格度验证、状态机不变式和 Evidence Bundle 缺口
...
| [`lessons-learned/hello/docs/spdd-report/SUMMARY.md`](lessons-learned/hello/docs/spdd-report/SUMMARY.md), [`lessons-learned/hello/docs/spdd-report/spdd/01-spdd-main-article.md`](lessons-learned/hello/docs/spdd-report/spdd/01-spdd-main-article.md), [`lessons-learned/hello/docs/spdd-report/tools/10-spdd-ecosystem.md`](lessons-learned/hello/docs/spdd-report/tools/10-spdd-ecosystem.md) | Structured Prompt-Driven Development 归档 | REASONS Canvas、三大技能、OpenSPDD、SDD 生态分层 |
| [`lessons-learned/hello/docs/formal-verification-research/research.md`](lessons-learned/hello/docs/formal-verification-research/research.md), [`lessons-learned/hello/docs/formal-verification-research/pdfs/`](lessons-learned/hello/docs/formal-verification-research/pdfs/) | 形式化验证与形式化规格研究 | TLA+、Alloy、Lean、Coq、Isabelle/HOL、seL4、CompCert、AI 辅助证明 |
| [`lessons-learned/hello/docs/reports/2026/mas/00-INDEX.md`](lessons-learned/hello/docs/reports/2026/mas/00-INDEX.md) | 多 Agent 工程与协议归档 | MAS、MCP、A2A、协议生态 |
| [`lessons-learned/hello/docs/reports/research-report.md`](lessons-learned/hello/docs/reports/research-report.md), [`lessons-learned/hello/docs/research/ai-market/2026_AI_Agent_Market_Analysis_Deep_Research.md`](lessons-learned/hello/docs/research/ai-market/2026_AI_Agent_Market_Analysis_Deep_Research.md) | 中文 AI Agent 研究综述与市场材料 | Agent Memory、Agent Evolution、Agent Teams、Harness 治理 |
...
本次增补将 hello 项目中的 AI Markdown 归为八组，引用材料已按 hello 原相对路径复制到 [`lessons-learned/hello/`](lessons-learned/hello/)，避免把 `tmp/` 下镜像仓库和 `docs/reports/2026/` 主归档重复计数为独立结论：
...
| 形式化验证 | [`hello/docs/formal-verification-research/`](lessons-learned/hello/docs/formal-verification-research/) | TLA+、Alloy、Lean、Coq、Isabelle/HOL、证明助手、工业验证案例 | 校准高严格度任务的机器可检查验证和状态机不变式路线 |
| MAS / 协议 | [`hello/docs/reports/2026/mas/`](lessons-learned/hello/docs/reports/2026/mas/) | Supervisor、Adaptive Network、Swarming、MCP+A2A+ACP | 校准 relay-teams 的 MCP/ACP/A2A 边界 |
| 企业 Agent 平台 | [`hello/docs/reports/2026/google/`](lessons-learned/hello/docs/reports/2026/google/), [`hello/docs/reports/2026/google-cloud-next/`](lessons-learned/hello/docs/reports/2026/google-cloud-next/) | Agentic Enterprise、平台化 Agent、上下文和基础设施 | 校准企业部署和多 Provider 管理方向 |
| 中文综述与市场材料 | [`hello/docs/reports/research-report.md`](lessons-learned/hello/docs/reports/research-report.md), [`hello/docs/research/ai-market/`](lessons-learned/hello/docs/research/ai-market/), [`hello/docs/presentations/ai-*`](lessons-learned/hello/docs/presentations/) | Agent Memory、Agent Evolution、Agent Teams、AI Agent 市场、企业落地 | 补强 FE-1、AO-2、FE-3、SG-1 等原借鉴点的证据 |
| 治理与 AI-native 组织 | [`hello/The_Playbook_For_Building_An_AI_Native_Company.md`](lessons-learned/hello/The_Playbook_For_Building_An_AI_Native_Company.md) | 闭环组织、可查询工件、跨 Provider 治理包、证据纪律 | 校准产品层治理和组织级知识工程 |
...
| **OP-8** | 跨 Provider 治理包与 A2A 五元组 handoff | 中 | [`hello/The_Playbook_For_Building_An_AI_Native_Company.md`](lessons-learned/hello/The_Playbook_For_Building_An_AI_Native_Company.md)、provider-native instruction conventions | relay-teams 能读取 AGENTS.md/CLAUDE.md/GEMINI.md，但没有统一生成和版本化治理包 | 将硬约束、协作标准、质量纪律、知识工程拆成版本化 governance pack；A2A/role handoff 统一 What/Why/Tradeoff/Open Questions/Next Action |
...
**已完成第一阶段闭环（2026-04-30）**。编排配置已从“角色列表 + prompt 约定”扩展为显式 DAG contract，`OrchestrationPreset.graph` 可声明 `nodes`、`edges`、`max_parallel_tasks` 和 `final_response_node_id`；`RunTopologySnapshot` 会携带 `orchestration_graph`，Coordinator 在选中 graph preset 时自动按依赖创建 delegated task、执行 fan-out、等待 join，并把上游结果注入下游 node objective。普通 preset 仍保留现有 Coordinator 动态委派循环，避免破坏既有默认编排和测试基线。
...
- 聚焦测试已覆盖 graph contract 校验、run topology 持久化、Coordinator fan-out+join 调度、系统配置 API 和设置页 graph 保真。
...
仓库中仍存在非编排热路径的 sync/async 双路径，主要分布在 session/admin/gateway/automation 等外部配置和管理面。后续阶段应继续将这些边界迁移为 async 服务方法，并把同步入口限制在 CLI/进程入口。
...
已确立"异步优先"架构原则，并完成编排核心路径第一阶段迁移。后续应按仓储归属继续清理剩余 `_call_sync_async` 包装器和 server router 的 `call_maybe_async` 管理面桥接；同步入口仅应保留在 CLI 等必要边界，通过 `asyncio.run()` 调用 async API。
...
*本文件整合自 cross-reference-analysis.md（25 借鉴点）、markdown-research-points.md（35 研究点 + 38 源文件）、validation-report.md（验收修正）三份原始报告，并在 2026-04-28/29 增补 hello AI Markdown、OpenAI Symphony 状态机、SPDD 和形式化验证复盘；本文件直接引用的 hello 来源材料已复制到 `lessons-learned/hello/`。所有原 25 个借鉴点已逐一提取、交叉引用来源、标注验收修正；新增 OP 借鉴点以当前源码状态为准。*
