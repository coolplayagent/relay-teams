...
- Show routed skill candidates in the per-turn user prompt instead of the system prompt.
...
- Dynamic skill candidates may only appear in the per-turn user prompt appendix.
...
Each skill is indexed into retrieval scope `scope_kind=skill`, `scope_id=skills`.
...
- If authorized skill count is `<= 8`, use passthrough and keep all authorized skills visible.
...
They must not contain routed skill candidates or any objective-specific skill text.

### User Prompt

The user prompt is the task objective plus an optional `## Skill Candidates` appendix.
That appendix contains routed `name + description` entries only.
...
The preview `user_prompt` returns the final turn prompt text, including the skill-candidate appendix.
