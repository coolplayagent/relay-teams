...
新增 system prompt section 时必须遵守稳定前缀合同：不得把 workspace、SSH、时间或 objective-dependent 内容插入 role prompt、runtime rules、skill catalog、capability summary 之前。详细规则见 `docs/modules/prompt/system-prompt-layering.md`。
