...
当前仓库里的 `exec session` 运行时已经按这个模型对齐到统一会话抽象，而不是继续把 “foreground shell” 和 “background terminal” 拆成两套实现：
...
- Windows:
  ...
  - 如果 Windows 主机不满足 ConPTY 运行条件，则明确拒绝 TTY exec session，但仍允许非 TTY exec session
- 审批缓存:
  - 规范化不再只认 `bash -lc`
